<section>
    <div class="about">
        @if ($abouts->isEmpty())
        <div class="alert alert-primary d-flex align-items-center mt-5" role="alert">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
            class="bi bi-exclamation-triangle-fill flex-shrink-0 me-2" viewBox="0 0 16 16"
            role="img" aria-label="Warning:">
              <path d="M8.982 1.566a1.13 1.13 0 0 0-1.96
              0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0
              1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0
              .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1
              0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1
              0 2 1 1 0 0 1 0-2z"/>
            </svg>
            <div>
             Please add some information to display. click <a href="{{ route('login') }}" wire:navigate>here</a> to add data
            </div>
          </div>
        @endif
        <div class="about_section" id="About">
            @foreach ($abouts as $about)
            <div class="about_img">
                <img src="{!! asset('storage/'.$about->image) !!}" alt="avatar" loading="lazy" class="pe-2">
            </div>
            <div class="about_details">
                <h1><b style="font-weight: 600">Let me</b> introduce myself</h1>
                <p>{{ $about->description }}</p>
                <div class="btns mt-4">
                    <a href="/contact" class="btn6 btn1" wire:navigate>Contact me</a>
                    <a href="/prices" class="btn6 btn2" wire:navigate>Hire me</a>
                </div>
            </div>
            @endforeach
        </div>
    </div>
    @include('livewire.common.components.footer')
</section>
