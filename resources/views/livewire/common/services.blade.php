<section>
    <section class="amazing_feature">
        <div class="service-container">

            <div class="row">
                <div class="col-md-12 text-center heading-main">
                    <h2 class="heading-services">Why Choose me</h2>
                    <div class="separator"><i class="bx bx-image-alt below-line-about-icon"></i></div>
                </div>
            </div>
            <div class="row">
                @if ($services->isEmpty())
                <div class="alert alert-primary d-flex align-items-center" role="alert">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                    class="bi bi-exclamation-triangle-fill flex-shrink-0 me-2" viewBox="0 0 16 16" role="img" aria-label="Warning:">
                      <path d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091
                       1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982
                       1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0
                       1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0
                        1 1 0 2 1 1 0 0 1 0-2z"/>
                    </svg>
                    <div>
                     Please add some information to display.
                    </div>
                  </div>
                @endif
                @foreach ($services as $service)
                    <div class="col-md-4 col-sm-6 col-xs-12">
                        <div class="single_feature">
                            <i class="{{ $service->icon }}"></i>
                            <h3>{{ $service->heading }}</h3>
                            <p>{{ $service->description }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
        </div>
    </section>
    @include('livewire.common.components.footer')
</section>
